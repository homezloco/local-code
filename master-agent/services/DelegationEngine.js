...
async function cancelDelegationForTask(taskId, reason = 'Cancelled by user') {
  ...
  for (const delegation of delegations) {
    // Abort any active execution for this delegation
    if (activeExecutions.has(delegation.id)) {
      try {
        logger.info(`Aborting execution for delegation ${delegation.id}`);
        activeExecutions.get(delegation.id).abort();
        activeExecutions.delete(delegation.id);
      } catch (e) {
        logger.warn(`Failed to abort delegation ${delegation.id}: ${e.message}`);
      }
    }
